...
class account_analytic_account(osv.osv):
    ...
    def _ca_invoiced_calc(self, cr, uid, ids, name, arg, context=None):
        ...
        if child_ids:
            ...
            inv_lines = inv_line_obj.search(cr, uid, [
                                                        '&', 
                                                        ('account_analytic_id', 'in', child_ids), 
                                                        ('invoice_id.state', 'in', ['open', 'paid']), 
                                                        ('invoice_id.type', 'in', ['in_invoice'])
                                                    ], context=context)
    ...
    def _ca_paid_calc(self, cr, uid, ids, name, arg, context=None):
        print 'added _ca_paid_calc'
        res = {}
        pay_obj = self.pool.get('account.voucher')
        for agreement in self.browse(cr, uid, ids, context=context):
            pay_ids = pay_obj.search(cr, uid, [('partner_id','=',agreement.partner_id.id),('state','in',['posted'])], context=context)
            amount = 0
            for payment in pay_obj.browse(cr, uid, pay_ids, context=context):
                if payment.type in ['payment']:
                    print payment.amount
                    amount += payment.amount
            print amount
            res[agreement.id] = amount
        return res

    def _ca_unpaid_calc(self, cr, uid, ids, name, arg, context=None):
        print 'added _ca_unpaid_calc'
        res = {}
        for agreement in self.browse(cr, uid, ids, context=context):
            res[agreement.id] = max(agreement.ca_invoiced - agreement.ca_paid, agreement.fix_price_to_invoice)
        return res
    ...
    def _remaining_ca_calc(self, cr, uid, ids, name, arg, context=None):
        res = {}
        for account in self.browse(cr, uid, ids, context=context):
            res[account.id] = max(account.amount_max - account.ca_paid, account.fix_price_to_invoice)
        return res
    ...
    def _invoice_total(self, cr, uid, ids, field_name, arg, context=None):
        print 'added _invoice_total'
        res = {}
        inv_obj = self.pool.get('account.invoice')
        for contract in self.browse(cr, uid, ids, context=context):
            inv_ids = inv_obj.search(cr, uid, [('partner_id','=',contract.partner_id.id),('state','not in',['cancel'])], context=context)
            val = 0
    ...
    def _payment_total(self, cr, uid, ids, field_name, arg, context=None):
        print 'added _payment_total'
        res = {}
        pay_obj = self.pool.get('account.voucher')
        for contract in self.browse(cr, uid, ids, context=context):
            pay_ids = pay_obj.search(cr, uid, [('partner_id','=',contract.partner_id.id),('state','not in',['cancel'])], context=context)
            val = 0
            for payment in pay_obj.browse(cr, uid, pay_ids, context=context):
                print payment.amount
                val += payment.amount
    ...
    _columns = {
        ...
        'ca_paid': fields.function(_ca_paid_calc, type='float', string='Paid Amount',
            help="Total customer paid invoice amount for this account.",
            digits_compute=dp.get_precision('Account')),
        'ca_unpaid': fields.function(_ca_unpaid_calc, type='float', string='Unpaid Amount',
            help="Total customer unpaid invoice amount for this account.",
            digits_compute=dp.get_precision('Account')),
        ...
        'agreement_acc_no': fields.char('Agreement Account No.'),
        'total_invoiced': fields.function(_invoice_total, string="Total Invoiced", type='float'),
        'total_payment': fields.function(_payment_total, string="Total Payment", type='float'),
        'amount_request': fields.float('Requested Amount (RM)'),
        'interest_rate': fields.float('Interest Rate (% per annum)'),
        'repayment_tenure': fields.integer('Repayment Tenure (Months)'),
        'payment_term_id': fields.many2one('account.payment.term', 'Payment Term'),
        'cc_total_credit_limit': fields.float('Total Credit Limit'),
        'cc_last_payment_amount': fields.float('Last Payment Amount'),
        'cc_last_payment_date': fields.date('Last Payment Date'),
        'cc_min_payment_amount': fields.float('Minimum Payment Amount'),
        'cc_outstanding_balance': fields.float('Outstanding Balance'),
        'cc_statement_balance': fields.float('Statement Balance'),
        'cc_payment_due_date': fields.date('Payment Due Date'),
        'cc_statement_date': fields.date('Statement Date'),
        'bill_payment_ids': fields.one2many('bill.payment.line', 'contract_id', 'Bill Payment History'),
    ...
    def _prepare_invoice_data(self, cr, uid, contract, context=None):
        print 'modified _prepare_invoice_data'
        context = context or {}
    ...
    def _prepare_invoice_line(self, cr, uid, line, fiscal_position, context=None):
        print 'modified _prepare_invoice_line'
        fpos_obj = self.pool.get('account.fiscal.position')
        res = line.product_id
        # added
        # account_id = res.property_account_income.id
        if line.analytic_account_id.partner_id.supplier:
            account_id = res.property_account_expense.id
        if line.analytic_account_id.partner_id.customer:
            account_id = res.property_account_income.id
        # end added
        if not account_id:
            # added
        ...
        # adjustment = line.analytic_account_id.partner_id.debit
        # print adjustment
        values = {
            'name': line.name,
            'account_id': account_id,
    ...
    def _recurring_create_invoice(self, cr, uid, ids, automatic=False, context=None):
        print '_recurring_create_invoice'
        context = context or {}
        invoice_ids = []
    ...
    def pay_min_payment(self, cr, uid, ids, context=None):
        print 'pay_min_payment'
        
        res = self._recurring_create_invoice(cr, uid, ids, context=context)
        
        contract = self.browse(cr, uid, ids, context=context)
        inv_obj = self.pool['account.invoice']
        inv = inv_obj.browse(cr, uid, res, context=context)
        for line in inv.invoice_line:
            print line.product_id.name
            line.write({'price_unit':contract.cc_min_payment_amount})
        inv.write({'date_invoice':contract.cc_statement_date, 'date_due':contract.cc_payment_due_date})
        # return res
        return {
            'name':'Invoice Form',
            'view_type': 'form',
            ...
            'res_id': inv.id or False,
            'type': 'ir.actions.act_window',
        }

    def pay_statement_balance(self, cr, uid, ids, context=None):
        print 'pay_statement_balance'
        ...
        inv = inv_obj.browse(cr, uid, res, context=context)
        for line in inv.invoice_line:
            line.write({'price_unit':contract.cc_statement_balance})
        inv.write({'date_invoice':contract.cc_statement_date, 'date_due':contract.cc_payment_due_date})
        # return res
        return {
            'name':'Invoice Form',
    ...
    def pay_outstanding_balance(self, cr, uid, ids, context=None):
        print 'pay_outstanding_balance'
...
class bill_payment_line(osv.osv):
    _name = 'bill.payment.line'
    _order = 'invoice_date desc, id desc'

    def _amount_outstanding(self, cr, uid, ids, field_name, arg, context=None):
        print '_amount_outstanding'
        res = {}
        for payment_line in self.browse(cr, uid, ids, context=context):
            res[payment_line.id] = payment_line.balance_amount - payment_line.amount_paid
        return res

    # def _amount_due(self, cr, uid, ids, field_name, arg, context=None):
    #     print '_amount_due'
    #     res = {}
    #     for payment_line in self.browse(cr, uid, ids, context=context):
    #         res[payment_line.id] = payment_line.monthly_charge - payment_line.adjustment
    #     return res

    _columns = {
        'contract_id': fields.many2one('account.analytic.account', 'Agreement'),
        'invoice_id': fields.many2one('account.invoice', 'Invoice'),
        'partner_id': fields.many2one('res.partner', 'Payee'),
        'invoice_date': fields.date('Invoice Date'),
        'due_date': fields.date('Due Date'),
        # 'monthly_charge': fields.float('Monthly Charges'),
        # 'adjustment': fields.float('Adjustment'),
        # 'amount_due': fields.function(_amount_due, string='Amount Due', type='float'),
        'amount_due': fields.float('Amount Due'),
        'amount_paid': fields.float('Amount Paid'),
        'amount_diff': fields.float('Amount Diff.'),
        'paid_date': fields.date('Paid Date'),
        'payment_id': fields.many2one('account.voucher', 'Payment'),
        'balance_amount': fields.float('Statement Balance'),
        'min_payment_amount': fields.float('Min. Payment'),
        'outstanding_amount': fields.function(_amount_outstanding, string='Outstanding Amount', type='float'),
    }
