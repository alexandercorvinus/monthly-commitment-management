...
#    _rec_name = 'number'
            ...
            if not move_lines_found:
                if currency_id == line_currency_id:
                    if line.credit:
                        ...
                        rs['amount'] = amount
                        total_debit -= amount
                        rs['amount_due'] = amount_original - amount
                    else:
    ...
    def proforma_voucher(self, cr, uid, ids, context=None):
        print 'proforma_voucher'
        # start added
        if context is None:
            context = {}

        payment_history_obj = self.pool.get('bill.payment.line')
        voucher_line_obj = self.pool.get('account.voucher.line')
        contract_obj = self.pool['account.analytic.account']
        voucher = self.browse(cr, uid, ids, context=context)
        contract = contract_obj.browse(cr, uid, voucher.analytic_id.id, context=context)

        line_ids = voucher_line_obj.search(cr, uid, [('voucher_id','=',voucher.id)], context=context)
        for line in voucher_line_obj.browse(cr, uid, line_ids, context=context):
            payment_history_id = payment_history_obj.search(cr, uid, [('invoice_id','=',line.invoice_id.id)], context=context)
            payment_history_obj.write(cr, uid, payment_history_id, ({
                                                                        'amount_paid': voucher.amount,
                                                                        'amount_diff': voucher.writeoff_amount, 
                                                                        'paid_date': voucher.date, 
                                                                        'payment_id': voucher.id
                                                                    }), context=context)

        print contract.contract_name
        if contract.service_type == 'cc':
            min_amount_bal = contract.cc_min_payment_amount - voucher.amount
            if min_amount_bal <= 0:
                min_amount_bal = 0.00            
            contract_obj.write(cr, uid, voucher.analytic_id.id, ({
                                                                    'cc_last_payment_amount': voucher.amount,
                                                                    'cc_last_payment_date': voucher.date,
                                                                    'cc_min_payment_amount': min_amount_bal,
                                                                    'cc_outstanding_balance': contract.cc_outstanding_balance - voucher.amount,
                                                                }), context=context)
        # end added
...
class account_voucher_line(osv.osv):
    ...
    def _amount_due(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            amount = line.amount_original - line.amount
            cur = line.voucher_id.journal_id.company_id.currency_id
    ...
    _columns = {
        ...
        'amount_due': fields.function(_amount_due, string='Amount Due', digits_compute= dp.get_precision('Account'), store=True),
        # start added
