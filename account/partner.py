...
class res_partner(osv.osv):
    ...
    _columns = {
        ...
        'account_receivable_code': fields.char('Account Receivable Code'),
        'account_payable_code': fields.char('Account Payable Code'),
    }
    ...
    def create(self, cr, uid, vals, context=None):
        print 'upgrade existing create function'
        
        account_obj = self.pool.get('account.account')
        data_obj = self.pool.get('ir.model.data')

        #sequence for Supplier Account Payable
        if vals.get('supplier'):

            tmp_account_ids = account_obj.search(cr, uid, [('code','=','120')])
            tmp_account = account_obj.browse(cr, uid, tmp_account_ids)

            account_type_id = data_obj.xmlid_to_res_id(cr, uid, 'account.data_account_type_payable')

            for idx in ascii_uppercase:
                if str(vals['name'][:1].upper()) == idx:
                    if not vals['parent_id']:
                        #call the respective sequence
                        vals['account_payable_code'] = self.pool.get('ir.sequence').get(cr, uid, 'account.payable.sequence') or '/'
                        account_id = account_obj.create(cr, uid, {
                                                                    'code':vals['account_payable_code'],
                                                                    'name':vals['name'],
        ...
        #sequence for Customer Account Receivable
        if vals.get('customer'):

            tmp_account_ids = account_obj.search(cr, uid, [('code','=','110')])
            tmp_account = account_obj.browse(cr, uid, tmp_account_ids)

            account_type_id = data_obj.xmlid_to_res_id(cr, uid, 'account.data_account_type_receivable')

            for idx in ascii_uppercase:
                if str(vals['name'][:1].upper()) == idx:
                    if not vals['parent_id']:
                        #call the respective sequence
                        vals['account_receivable_code'] = self.pool.get('ir.sequence').get(cr, uid, 'account.receivable.sequence') or '/'
                        account_id = account_obj.create(cr, uid, {
                                                                    'code':vals['account_receivable_code'],
                                                                    'name':vals['name'],
                                                                    'parent_id':tmp_account.id,
                                                                    'type':'receivable',
                                                                    'user_type':account_type_id,
        ...
        res = super(res_partner, self).create(cr, uid, vals, context=ctx)
        return res
