...
class account_invoice(models.Model):
    ...
    def invoice_validate(self):
        ...
        # start added
        payment_history_obj = self.env['bill.payment.line']
        for invoice in self:
            payment_history = payment_history_obj.search([
                                                            ('partner_id','=',invoice.partner_id.id),
                                                            ('contract_id','=',invoice.account_analytic_id.id)
                                                        ], limit=1, order='invoice_date DESC')
            if invoice.account_analytic_id.service_type == 'cc':
                payment_history_obj.create({
                                                'contract_id': invoice.account_analytic_id.id,
                                                'invoice_id': invoice.id,
                                                'partner_id': invoice.partner_id.id,
                                                'invoice_date': invoice.date_invoice,
                                                'due_date': invoice.date_due,
                                                # 'amount_due': invoice.amount_total - payment_history.amount_diff,
                                                # 'monthly_charge': invoice.amount_total,
                                                # 'adjustment': payment_history.amount_diff,
                                                'balance_amount': invoice.account_analytic_id.cc_outstanding_balance,
                                                'min_payment_amount': invoice.account_analytic_id.cc_min_payment_amount
                                            })
            else:
                payment_history_obj.create({
                                                'contract_id': invoice.account_analytic_id.id,
                                                ...
                                                # 'monthly_charge': invoice.amount_total,
                                                # 'adjustment': payment_history.amount_diff,
                                                'balance_amount': invoice.account_analytic_id.remaining_ca,
                                                'min_payment_amount': invoice.amount_total - payment_history.amount_diff
                                            })
